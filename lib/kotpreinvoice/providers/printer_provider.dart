...
//       print("✅ PrinterProvider fully initialized");
...
//       if (existingIndex != -1) {
//         print(
//           "⚠️ Printer with name '${printer.name}' already exists at index $existingIndex",
//         );
//         return;
//       }
...
//       final existingIndex = _printers.indexWhere(
//         (p) => p.name == updatedPrinter.name,
//       );
//       if (existingIndex != -1 && existingIndex != index) {
//         print(
//           "⚠️ Printer with name '${updatedPrinter.name}' already exists at index $existingIndex",
//         );
//         return;
//       }
...
//       _savePrintersToHive();
//       print(
//         "✅ Updated printer at index $index: ${updatedPrinter.name} (IP: ${updatedPrinter.ipAddress})",
//       );
//       notifyListeners();
//     } else {
...
//     try {
//       return _printers
//           .firstWhere((printer) => printer.type == 'Overall')
//           .ipAddress;
//     } catch (e) {
...
class PrinterProviderDine with ChangeNotifier {
  ...
  //     });
  //   } catch (e) {
  //     print("❌ Error opening Hive box 'KOTprinters': $e");
  //   }
  // }
  ...
  Map<String, dynamic>? getPrinterForItem(String itemName) {
    try {
      if (itemName.isEmpty) {
        print("⚠️ getPrinterForItem: Empty item name provided.");
        return null;
      }
      ...
      for (final printer in _printers) {
        for (final item in printer.items) {
          if (item.trim().toLowerCase() == normalizedItem) {
            ...
            print("✅ Found printer for item '$itemName' → $printerInfo");
            return printerInfo;
          }
        }
      }

      print("⚠️ No printer found for item '$itemName'");
      return null;
    } catch (e, st) {
      print("❌ Error in getPrinterForItem('$itemName'): $e");
      print("📜 StackTrace:\n$st");
      return null;
    }
  }
