                        ...
                        child: GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            Future<void> _addDirectlyToCart() async {
                              ...
                              final varianceDataFromMap = itemData?['variance']?[variance.varianceName];
                              ...
                              final Map<String, dynamic> newItem = {
                                ...
                                'quantity': 1.0,
                                'totalPrice': 1.0 * variance.varianceDefaultPrice,
                                'id': itemId,
                                'itemName': itemName,
                              };
                            ...
                            return GestureDetector(
                              onTap: () async {
                                ...
                                final String varianceUOM = GlobalDataManager().branchwiseItems['data'].entries
                                    .firstWhere(
                                      (entry) =>
...
Widget buildVarianceTile(Variance variance, Map<String, dynamic> varianceData, String varianceUOM) {
  ...
  return RepaintBoundary(
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isSelected ? CustomColors.blueColor : CustomColors.whiteColor),
      ),
      alignment: Alignment.centerLeft,
      child: Column(
        children: [
              ...
              Expanded(
                child: CustomText(
                  ...
                  style: TextStyle(
                    fontFamily: "Poppins",
                    fontSize: 14,
                    color: isSelected ? CustomColors.whiteColor : CustomColors.black.withOpacity(0.7),
                  ),
                ),
              ),
              CustomText(
      ...
      builder: (context) {
        ...
        return NumericCalculator(
          ...
          onValueSelected: (weight) async {
            ...
            if (weight > 0) {
              isAdding = true;
              await _addWeightItemToCart(context, itemName, varianceName, price, weight, onWeightSelected);
              Provider.of<CurrentSaleProvider>(context, listen: false).loadCartItems();
              disposeLargeObjects(context);
              // Navigator.of(context).pop(); // Close numeric calculator only
              isAdding = false;
            } else {
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(const SnackBar(content: Text("Invalid weight. Please enter a valid weight.")));
            }
          },
        );
      },
        ...
        actions: <Widget>[
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              double weight = Provider.of<BluetoothProvider2>(context, listen: false).weight;

              if (weight <= 0) {
                ScaffoldMessenger.of(
                  context,
                ).showSnackBar(const SnackBar(content: Text("Invalid weight. Please wait for a valid weight reading.")));
                return;
              }
            ...
            child: const CustomText(
              text: 'Add',
              style: TextStyle(fontFamily: 'Poppins',fontSize: 16.0, fontWeight: FontWeight.bold, color: CustomColors.whiteColor),
            ),
          ),
        ],
      );
...
Future<void> _addWeightItemToCart(
  ...
  try {
    final itemData = GlobalDataManager().branchwiseItems['data']?[itemName];
    if (itemData == null || itemData['variance'] == null || itemData['variance'][varianceName] == null) {
    ...
    Map<String, dynamic> newItem = {
      'itemData': {
        ...itemData['item'],
        'itemId': itemData['item']['itemId']?.toString() ?? itemData['item']['hsnCode']?.toString(),
        'itemName': itemName,
      ...
      'varianceData': {
        ...varianceData,
        'varianceName': varianceName,
        'variance_Defaultprice': price is String ? double.tryParse(price as String) ?? 0.0 : price,
    ...
    await Provider.of<CurrentSaleProvider>(context, listen: false).addItemToCart(processedItem);

    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text("Added $varianceName (${weight.toStringAsFixed(3)} kg) to cart")));
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("Error adding to cart: $e"), backgroundColor: Colors.red, duration: const Duration(seconds: 2)),
    );
  }
}

void showQuantityDialog(BuildContext context, String itemName, String varianceName, double price, Function(double) onAddToCart) {
  double quantity = 1.0;
  final TextEditingController _controller = TextEditingController(text: quantity.toInt().toString());
              ...
              TextButton(
                onPressed: () async {
                  ...
                  if (itemData == null || itemData['variance'] == null || itemData['variance'][varianceName] == null) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(
                      SnackBar(
                        content: Text("Item or variance data not found for $varianceName."),
                        backgroundColor: Colors.red,
                  ...
                  print("DEBUG: showQuantityDialog - Selected quantity: $quantity, System stock: $systemStock");

                  if (quantity > systemStock) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(
                      SnackBar(
                        content: Text("Selected quantity ($quantity) exceeds available stock ($systemStock)."),
                        backgroundColor: Colors.red,
                        duration: const Duration(seconds: 2),
                      ),
                    );
                  ...
                  try {
                    ...
                    final currentSaleProvider = Provider.of<CurrentSaleProvider>(dialogContext, listen: false);
                    await currentSaleProvider.addItemToCart(processedItem);
                    await currentSaleProvider.loadCartItems();

                    final quantityProvider = Provider.of<QuantityProvider>(dialogContext, listen: false);
                    ...
                    print("DEBUG: showQuantityDialog - Error accessing providers: $e");
                    ScaffoldMessenger.of(dialogContext).showSnackBar(
                      SnackBar(
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text("Please connect to a Bluetooth device to proceed."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        shape: const RoundedRectangleBorder(borderRadius: BorderRadius.zero),
        backgroundColor: CustomColors.whiteColor,
        title: Text(varianceName, style: const TextStyle(fontFamily: 'Poppins',fontSize: 18.0, fontWeight: FontWeight.bold)),
        content: Consumer<BluetoothProvider2>(
          builder: (context, bluetoothProvider2, child) {
            double weight = bluetoothProvider2.weight;
            return Column(
              mainAxisSize: MainAxisSize.min,
              children: [Text("Weight: ${weight.toStringAsFixed(2)} kg", style: const TextStyle(fontFamily: 'Poppins',fontSize: 24))],
            );
          },
        ),
        actions: <Widget>[
          ElevatedButton(
            onPressed: () async {
              ...
              if (weight > systemStock) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text("Selected weight ($weight kg) exceeds available stock ($systemStock kg)."),
                    backgroundColor: Colors.red,
                    duration: Duration(seconds: 1),
                  ),
                );
