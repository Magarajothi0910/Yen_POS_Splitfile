...
class _SplitBillDialogState extends State<SplitBillDialog> {
      ...
      showDialog(
        ...
        builder: (context) {
          TextEditingController qtyController = TextEditingController(
            text: qtyToMove.toString(),
          );
          return AlertDialog(
            backgroundColor: CustomColors.whiteColor,
            title: Text(
              'Split Quantity for ${item['varianceData']['varianceName']}',
            ),
            content: TextField(
              ...
              decoration: InputDecoration(
                labelText: 'Quantity to move (1 to $qtyToMove)',
  ...
  Map<String, dynamic> _deepCopyItem(Map<String, dynamic> item) {
    return {
      ...
      'uniqueId':
          '${DateTime.now().millisecondsSinceEpoch}-${item['itemCode']}',
    };
  }
  ...
  void deleteTicket(int index) {
    if (tickets.length <= 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("At least one ticket must remain.")),
      );
      return;
    }
                    ...
                    onPressed: () {
                      ...
                      if (nonEmptyTickets.isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'At least one ticket must have items!',
                            ),
                            backgroundColor: CustomColors.redColor,
                            duration: Duration(seconds: 2),
                          ),
...
class TicketPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DragTarget<Map<String, dynamic>>(
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if (isWeightItem) {
                            ...
                            displayText =
                                '${item['varianceData']?['varianceName'] ?? "Unknown"} ${weight.toStringAsFixed(3)} kg';
                          } else {
                            int quantity =
                                (item['quantity'] as num?)?.toInt() ?? 0;
                            displayText =
                                '${item['varianceData']?['varianceName'] ?? "Unknown"} x $quantity';
                          }
                          ...
                          return Draggable<Map<String, dynamic>>(
                            data: {
                              'item': item,
                              'index': index,
                              'fromTicketIndex': currentTicketIndex,
                            },
                            feedback: Material(
                              elevation: 4,
                              borderRadius: BorderRadius.circular(8),
      ...
      onWillAccept: (data) =>
          data != null && data['fromTicketIndex'] != currentTicketIndex,
      onAccept: (data) {
        onItemDropped(data);
      },
    );
