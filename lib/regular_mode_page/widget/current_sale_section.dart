...
import 'package:yen_pos/Global/Widget/custom_button_reuse.dart';
...
class CurrentSaleSectionState extends State<CurrentSaleSection> {
  ...
  @override
  void initState() {
    Provider.of<PrinterProviderpos>(context, listen: false).initializeHive();
    super.initState();
  }
  ...
  final FocusNode _customerNumberfocusNode = FocusNode();
  final FocusNode _salesPersonfocusNode = FocusNode();
    ...
    return showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
            'Cannot go back.\n\n'
            'UPI/Card payment has already been processed.\n'
            'Please complete the remaining balance or contact support.',
  ...
  Widget build(BuildContext context) {
    ...
    // Check if ANY successful digital payment exists
    final bool hasSuccessfulDigitalPayment = stateProvider.isUpiPaid;
                    ...
                    children: [
                      Expanded(
                        ...
                        child: EmployeeSearch(
                          employeeController: _salesPersonController,
                          employeeFocusNode: _salesPersonfocusNode,
                          readOnly: false,
                        ),
                      ),
                      SizedBox(width: 5),
                      Expanded(
                        ...
                        child: CustomerSearchDropdown(
                          ...
                          focusNode: _customerNumberfocusNode,
                        ),
                      ),
                    ],
                  ),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return FutureBuilder(
                          future: Hive.openBox('items').then(
                            (lazyBox) =>
                                lazyBox.get('branchwiseItems_$locationId'),
                          ),
                            ...
                            if (branchwiseItems != null &&
                              ...
                              if (varianceMap != null) {
                                ...
                                if (varianceData != null &&
                                    ...
                                    varianceData['branchwise'][locationId] !=
                                        null) {
                                  systemStock =
                  ...
                  Center(
                    child: CustomButton(
                      ...
                      onPressed: () async {
                            ...
                            if (varianceMap != null) {
                              for (var v in varianceMap.values) {
                                if (v['varianceName']?.toString() ==
                                    varianceName) {
                                  availableStock =
                                      (v['branchwise']?[locationId]?['systemStock']
                                              as num?)
                                          ?.toDouble() ??
                                      0.0;
                                  break;
                        ...
                        showDialog(
                          context: context,
                          barrierDismissible: hasSuccessfulDigitalPayment
                              ? false
                              : true, // Prevent outside tap close completely
                          builder: (BuildContext dialogContext) {
                            return WillPopScope(
                              onWillPop: () async {
                                ...
                                // Check if ANY successful digital payment exists
                                final bool hasSuccessfulDigitalPayment =
                                    stateProvider.isUpiPaid ||
                                    stateProvider.isCardPaid ||
                                    // Add this if you track split payments successfully paid
                                    // _splitPayments.any((p) => p['paid'] == true) ||
                                    false;

                                if (hasSuccessfulDigitalPayment) {
                                  // Payment already done → BLOCK + show message
                                  showPaidDialog();
                                  return false; // ← Prevents closing
                                }

                                // No successful digital payment → show normal confirmation
                          ...
                          // This runs only when dialog is actually closed (allowed)
