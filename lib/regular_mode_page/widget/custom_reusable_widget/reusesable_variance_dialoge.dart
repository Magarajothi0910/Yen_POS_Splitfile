...
class _ReusableVarianceDialogState extends State<ReusableVarianceDialog> {
  ...
  double _calculateTotalWeight() {
    ...
    total += widget.variances.fold(
      0,
      (sum, variance) => sum + variance.varianceDefaultPrice,
    );
  ...
  void _insertText(String myText) {
    ...
    _controller.value = TextEditingValue(
      ...
      selection: TextSelection.collapsed(
        offset: textSelection.start + myText.length,
      ),
    );
  ...
  void _backspaceText() {
    ...
    if (selectionLength > 0) {
      ...
      final newText =
          text.substring(0, textSelection.start - 1) +
                      ...
                      onPressed: () {
                        setState(() {
                          if (variance['isPredefined']) {
                            ...
                            final target = widget.variances.firstWhere(
                              (v) => v.varianceName == variance['varianceName'],
                            );
                            target.varianceDefaultPrice = 0.0;
                          } else {
                            // Remove added variance
                            addedVariances.removeWhere(
                              (v) =>
                                  v['varianceName'] == variance['varianceName'],
                            );
                          }
  ...
  void _addNewVariance(String varianceName, double weight) {
    setState(() {
      addedVariances.add({"varianceName": varianceName, "weight": weight});
    });
  }
}
