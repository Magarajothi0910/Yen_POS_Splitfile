...
Widget buildHoldOrdersDropdown(BuildContext context, String currentTable, String currentSeat) {
  ...
  final filteredOrders = holdOrders.where((order) => order['table'] != currentTable || order['seat'] != currentSeat).toList();
  ...
  filteredOrders.sort((a, b) {
    ...
    final int tableNumA = int.tryParse(numberRegex.firstMatch(tableA)?.group(0) ?? '0') ?? 0;
    final int tableNumB = int.tryParse(numberRegex.firstMatch(tableB)?.group(0) ?? '0') ?? 0;
              ...
              child: Container(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      dense: true,
                      title: Text('$table - Seat $seat', style: const TextStyle(fontSize: 12)),
                      onTap: () {
                        hideOverlay();
                        final holdOrder = holdOrderProvider.loadHoldOrder(table, seat);
                        if (holdOrder != null) {
