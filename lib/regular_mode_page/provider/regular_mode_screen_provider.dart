...
import 'package:yen_pos/regular_mode_page/provider/image_service.dart';
...
class RegularModeProvider with ChangeNotifier {
  ...
  void filterVarianceNamesBySearchQuery(String query) {
    ...
    final List<String> matchedNames = originalItems
        .expand((item) => item['variances'] ?? [])
        .map((v) => (v['varianceName'] ?? "").toString())
        .where((name) {
          final cleanName = name.trim().replaceAll(' ', '').toLowerCase();
    ...
    matchedNames.sort((a, b) {
      final aClean = a.replaceAll(' ', '').toLowerCase();
      final bClean = b.replaceAll(' ', '').toLowerCase();

      final aStarts = aClean.startsWith(cleanQuery);
      final bStarts = bClean.startsWith(cleanQuery);

      if (aStarts && !bStarts) return -1;
      if (!aStarts && bStarts) return 1;

      return aClean.compareTo(bClean); // fallback alphabetical
    });

    filteredVarianceNames = matchedNames;
    notifyListeners();
  }
