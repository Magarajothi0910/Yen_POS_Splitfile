...
//     debugPrint("branchwise length : ${lazyBox.length}");
...
//         try {
//           var url =
//               // 'https://yenerp.com/fluttertestapi/branchwiseitems/over-all-data/?branch_alias=$branchAlias';
//               "https://yenerp.com/fluttertestapi/newbranchwiseitems/?location_id=LOC012";
...
//                 final localHiveStock = branchData['systemStock'];
//                 final localSystemStockSo =
//                     branchData['systemstockSo'];

//                 // Save to Hive
//                 final localStockKey = 'systemStock';
//                 final localSystemStockSoKey = 'systemstockSo';
...
//                   updatedBranchData['systemStock'] =
//                       localHiveStock;

//                   if (localSystemStockSo != null) {
//                     updatedBranchData['systemstockSo'] =
//                         localSystemStockSo;
//                   }
...
//             final result = checkVarianceItemCode("FG011", globals.locationId);
...
//         Uri.parse('https://yenerp.com/masteradminapi/locations/'),
//       );
...
//   Future<void> fetchAndStoreAdvancePercent({String? locationId}) async {
//     var client = http.Client();
...
//       var response = await client.get(
//         Uri.parse(
//           'https://yenerp.com/fluttertestapi/advanceamounts/advance/by-alias?aliasName=${globals.locationId}',
//         ),
//       );
...
//     final key = 'systemStock_$itemCode';
//     return box.get(key);
//   }
...
//    Future<void> fetchAndSaveOrderType() async {
//     const String apiUrl = 'https://yenerp.com/nextjstestapi/ordertypes/';
//     try {
//       final response = await http.get(Uri.parse(apiUrl));
//       if (response.statusCode == 200) {
//         final List<dynamic> orderTypeData = json.decode(response.body);

//         // Open Hive box and store employee data
//         var box = await Hive.openBox('orderType');
//         await box.put('OT', orderTypeData);
//       } else {}
//     } catch (e) {}
//   }
...
//       (b) =>
//           normalize(b['locationId']?.toString() ?? '') == normalize(aliasName),
//       orElse: () => null,
//     );
...
//         'aliasName': branch['aliasName']?.toString() ?? 'aliasName Not Available',
//       };
//     } else {
//       return null;
//     }
...
//           if (varianceData['itemCode'] == varianceItemCode) {
//             return [
//               {
//                 "itemData": itemData,
//                 "varianceData": varianceData,
...
class ItemProvider with ChangeNotifier {
  ...
  Future<void> fetchDataIfNeeded({String? branchAlias}) async {
    ...
    try {
      bool needToFetch = branchAlias != null;
      // &&
      // (globals.appType == 'server' || globals.appType == '');
      ...
      if (needToFetch) {
        try {
          var url =
              // 'https://yenerp.com/fluttertestapi/branchwiseitems/over-all-data/?branch_alias=$branchAlias';
              "https://yenerp.com/fluttertestapi/newbranchwiseitems/?location_id=$locationId";
          debugPrint("🌐 Fetching BranchWiseItems from URL: $url");
          ...
          if (response.statusCode == 200) {
            ...
            for (var itemEntry in branchwiseItems.entries) {
              ...
              for (var varianceEntry in variances.entries) {
                ...
                final localHiveStock = branchData['systemStock'];
                final localSystemStockSo = branchData['systemstockSo'];
                ...
                if (currentGlobalData != null) {
                  ...
                  updatedBranchData['systemStock'] = localHiveStock;

                  if (localSystemStockSo != null) {
                    updatedBranchData['systemstockSo'] = localSystemStockSo;
                  }
  ...
  Future<void> fetchAndStoreAdvancePercent({String? locationId}) async {
    ...
    try {
      var response = await client.get(
        Uri.parse(
          'https://yenerp.com/fluttertestapi/advanceamounts/advance/by-location?locationId=${globals.locationId}',
        ),
      );
  ...
  Future<Map<String, String>?> getBranchInfoFromAlias(String aliasName) async {
    ...
    if (branch != null) {
      return {
        ...
        'phone': branch['phoneNumber']?.toString() ?? 'Phone Not Available',
        'aliasName':
            branch['aliasName']?.toString() ?? 'aliasName Not Available',
      };
    } else {
      return null;
    }
