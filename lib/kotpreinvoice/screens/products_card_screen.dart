...
import '../../Global/Widget/weightSelector.dart';
...
class CartItemDialogState extends ChangeNotifier {
  ...
  List<String> type;
  bool toggleRemark; // Changed from List<bool> to bool
  String remark;
  List<int> configQty;
  ...
  void updateToggleRemark(bool value) {
    // Changed parameter
    toggleRemark = value;
    if (!value) {
      remark = "";
  ...
  void updateRemark(String remarkText) {
    // Changed parameter
    remark = remarkText;
    notifyListeners();
  }
...
class _ProductCardScreenState extends State<ProductCardScreen> {
  ...
  List<Product> _filteredProducts(List<Product> products, String query) {
    ...
    final matchedProducts = products.where((product) {
      return _normalize(product.name).contains(cleanQuery) ||
          _normalize(product.varianceName).contains(cleanQuery);
    }).toList();

    matchedProducts.sort((a, b) {
      final aName = _normalize(a.name);
      final bName = _normalize(b.name);
      final aVar = _normalize(a.varianceName);
      final bVar = _normalize(b.varianceName);

      final aStarts =
          aName.startsWith(cleanQuery) || aVar.startsWith(cleanQuery);
      final bStarts =
          bName.startsWith(cleanQuery) || bVar.startsWith(cleanQuery);

      if (aStarts && !bStarts) return -1;
      if (!aStarts && bStarts) return 1;

      final aContains = aName.contains(cleanQuery) || aVar.contains(cleanQuery);
      final bContains = bName.contains(cleanQuery) || bVar.contains(cleanQuery);
  ...
  List<Product> _filterBySubcategory(
    ...
    if (selectedCategory == 'Favorites') {
      ...
      final favoriteSet = quickAccessProvider.quickAccessProducts.toSet();

      return products
          .where((product) => favoriteSet.contains(product.varianceName))
          .toList();
    }
  ...
  void _addToCartWithEvent(
    ...
    try {
      ...
      if (displayStock == 0) {
        showCustomFlushbar(
          context,
          "${product.varianceName} is currently out of stock.",
  ...
  void _showEditDialog(Product product, CartProviderKOT cartProvider) {
    try {
      ...
      bool toggleRemark = cartProvider.getToggleRemark(
        productId,
      ); // Single boolean
      String remark = cartProvider.getRemark(productId); // Single string
      ...
      final dialogState = CartItemDialogState(
        ...
        variants: variants,
        toggleRemark: toggleRemark, // Single boolean
        remark: remark,
        configQty: configQty,
                        ...
                        children: [
                          ...List.generate(quantity, (i) {
                            if (i >= dialogState.addons.length ||
                              ...
                              // Removed toggleRemarks and remarks checks
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Column(
                                          ...
                                          children: [
                                            Switch(
                                              value: dialogState.toggleRemark,
                                              onChanged: (value) {
                          ...
                          if (dialogState.toggleRemark)
                            Padding(
                              ...
                              child: TextField(
                                ...
                                onChanged: (value) {
                                  final remarkIndex = dialogState.toggleRemark;
                                  if (remarkIndex != -1) {
                                    dialogState.updateRemark(value);
                                    cartProvider.updateCart(
                                      productId,
                                      dialogState.addons,
                                      dialogState.addonQuantities,
