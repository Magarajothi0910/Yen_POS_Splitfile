...
import 'package:yen_pos/Global/Widget/smartsearchtextfield.dart';
...
import 'package:yen_pos/transactionPage/widget/glass_card.dart';
...
Future<List<Map<String, dynamic>>> _getReturnedItemsForInvoice(
  ...
  try {
    ...
    debugPrint("Local Hive returns error: $e");
  }
  ...
  if (result.isEmpty) {
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> apiData = response.data as List<dynamic>;
        ...
        debugPrint("API returns fetch failed: ${response.statusCode}");
      }
    } catch (e) {
      debugPrint("API returns error: $e");
    }
  }
...
Widget _buildTransactionDetail(
  ...
  Future<void> _postSalesReturn(TransactionProvider transactionProvider) async {
    ...
    try {
      ...
      final previousInvoiceNo = transaction.invoiceNo?.toString();

      // ────────────────────────────────
      // Full clean refresh
      await transactionProvider.refreshAfterReturn();

      // Give Hive + provider a moment to settle (small but often necessary)
      await Future.delayed(const Duration(milliseconds: 150));

      if (previousInvoiceNo == null) return;

      // Try to find the same invoice again (by invoiceNo)
      final newIndex = transactionProvider.invoices.indexWhere((inv) {
        final invNo = inv['invoiceNo']?.toString();
        final returnFor = inv['originalInvoiceNo']
            ?.toString(); // if it's the return itself
        return invNo == previousInvoiceNo || returnFor == previousInvoiceNo;
      });

      if (newIndex != -1) {
        transactionProvider.selectedTransactionIndex = newIndex;
        print("→ Re-selected invoice at new index: $newIndex");
      } else {
        print("→ Could not find invoice $previousInvoiceNo after refresh");
        transactionProvider.selectedTransactionIndex = null;
      }
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    FutureBuilder<List<Map<String, dynamic>>>(
                                      ...
                                      key: ValueKey(
                                        'returned_${transaction.invoiceNo}_${transactionProvider.invoices.length}_${DateTime.now().millisecondsSinceEpoch ~/ 1000}',
                                      ),
                                      builder: (context, snapshot) {
                                        // Add a key to force rebuild when transaction changes
                                        return _buildReturnedItemsWidget(
                                          snapshot,
                                          transaction,
                                          editableItems,
                                        );
