...
Future<List<Map<String, dynamic>>> loadHoldOrdersFromHive() async {
  final Box holdOrdersBox = Hive.box('holdOrdersKOT');
  final List<Map<String, dynamic>> holdOrders = [];

  final String todayDate = DateFormat('dd-MM-yyyy').format(DateTime.now());

  try {
    for (final dynamic rawKey in holdOrdersBox.keys) {
      try {
        final dynamic orderData = holdOrdersBox.get(rawKey);

        // Basic validation
        if (orderData == null || orderData is! Map) continue;
        if (orderData['date'] != todayDate) continue;

        // Key format: table_seat
        final String key = rawKey.toString();
        final List<String> parts = key.split('_');

        final String table = parts.isNotEmpty && parts[0].isNotEmpty
            ? parts[0]
            : 'Unknown';
        final String seat = parts.length > 1 && parts[1].isNotEmpty
            ? parts[1]
            : 'A';

        final dynamic cartData = orderData['cart'];

        // Ensure cart is valid
        if (cartData == null || cartData is! Map || cartData.isEmpty) {
          debugPrint('⚠️ Skipping empty cart for $key');
          continue;
        }
        ...
        holdOrders.add({
          ...
          'cart': Map<String, dynamic>.from(cartData),
          'createdAt': orderData['createdAt'] ?? 0,
        });
      } catch (e, stack) {
        debugPrint('❌ Error processing hold order [$rawKey]: $e');
