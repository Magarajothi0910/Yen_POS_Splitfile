...
class ProductProvider with ChangeNotifier {
  ...
  Future<void> loadProductsFromHive() async {
    try {
      ...
      final dynamic fullWrapper = branchItems.get(
      ...
      dataMap.forEach((itemKey, value) {
        ...
        variances.forEach((varKey, varValue) {
          try {
            ...
            if (branchwise != null && branchwise.containsKey(locationId)) {
              final Map<String, dynamic> branchData = castToStringKeyedMap(
                branchwise[locationId],
              );

              final String stockKey = 'localHiveStock_$locationId';
        ...
        try {
          ...
          variances.forEach((varianceName, varianceData) {
            try {
              ...
              final branchData =
                  (varianceData['branchwise'] as Map?)?[locationId];

              if (itemCode != null && branchData != null) {
                final localStockKey = 'localStock_${locationId}_$itemCode';
                final localHiveStock = branchData['localHiveStock_$locationId'];
                ...
                if (localHiveStock != null) {
                  ...
                  final updatedBranchData = Map<String, dynamic>.from(
                    branchData,
                  )..['localHiveStock_$locationId'] = localHiveStock;
                  ...
                  updatedBranchwiseMap[locationId] = updatedBranchData;
  ...
  Future<void> fetchTablesAndSaveInHive(BuildContext context) async {
    ...
    try {
      await _hiveInitialized.future;
      final response = await dio.get(
        'https://yenerp.com/masteradminapi/tables/',
      );
  ...
  Future<void> fetchAddOnsAndSaveInHive() async {
    ...
    try {
      ...
      final response = await dio.get(
        'https://yenerp.com/masteradminapi/addons/',
  ...
  Future<void> fetchVariantsAndSaveInHive() async {
    ...
    try {
      ...
      final response = await dio
          .get(
            'https://yenerp.com/masteradminapi/kotvariants/',
            options: Options(headers: {"Accept": "application/json"}),
          )
