    ...
    try {
      ...
      if (scannedData.containsKey('ItemCode')) {
        ...
        if (result.isNotEmpty) {
          final itemData = result.first;
          // STOCK VALIDATION
          final stock = parseNum(itemData["varianceData"]["variance_Stock"] ?? 0);
          final scannedQty = parseNum(quantity);
          if (stock <= 0) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              backgroundColor: CustomColors.blueColor,
              content: Text("Item added: ${itemData['varianceData']['varianceName']}"),
              duration: const Duration(milliseconds: 800),
            ),
          );
        } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Invalid QR/barcode data."), duration: Duration(milliseconds: 800)),
        );
      }
    } catch (e) {
                                          ...
                                          Expanded(
                                            ...
                                            child: SingleChildScrollView(
                                              ...
                                              child: Row(
                                                children: [
                                                  TextButton(
                                                    onPressed: () => provider.setFavorite(true),
                                                    child: Text(
                                                      "Favorite",
                                                      style: TextStyle(
                                                        fontFamily: 'Poppins',
                                                  ...
                                                  TextButton(
                                                    onPressed: () => provider.setMixed(true),
                                                    child: Text(
                                                      "Mixed",
                                                      style: TextStyle(
                                                        fontFamily: 'Poppins',
