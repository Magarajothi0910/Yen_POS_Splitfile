                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, areaIndex) {
                      ...
                      final availableTables = areaTables.where((table) {
                        final mainTableNumber = table['tableNumber'].toString();

                        // Skip if this is the current table (assuming you're excluding it intentionally)
                        if (mainTableNumber == currentTable) {
                          return false;
                        }

                        // Get all related tables: main + any extra seats
                        final allRelatedTables = [mainTableNumber];
                        // final extras =
                        //     orderProvider
                        //         .extraTablesNotifier
                        //         .value[mainTableNumber] ??
                        //     [];
                        // allRelatedTables.addAll(extras);

                        // Check if ANY seat has orders > 0
                        bool hasAnyOrder = false;
                        for (final tableNum in allRelatedTables) {
                          final seatMatch = RegExp(
                            r'\(([A-Z])\)$',
                          ).firstMatch(tableNum);
                          final seat = seatMatch?.group(1) ?? 'A';
                        ...
                        return !hasAnyOrder; // Available only if no orders on any seat
                      }).toList();

                      if (availableTables.isEmpty) {
                        return const SizedBox(); // or some placeholder like "No available tables"
                      }
                        ...
                        children: [
                          ...
                          GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              debugPrint("table of transfer is :: $table");
