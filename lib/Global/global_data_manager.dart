...
class GlobalDataManager extends ChangeNotifier {
  ...
  void updateStockLive({
    ...
    final key = '$locationId|$varianceName';
    ...
    // Optional: log clearly
    print(
      "LIVE STOCK RECEIVED → $key = $effectiveStock (system: $systemStock, SO: $soStock)",
    );
    print(
      "CLIENT: Applying remote stock update → $varianceName = $effectiveStock (SO: $soStock)",
    );
  ...
  double getSystemStock(String locationId, String varianceName) {
    final key = '$locationId|$varianceName';
    return _systemStockCache[key] ?? -1;
  }

  // Get SO stock (committed/reserved)
  double getSoStock(String locationId, String varianceName) {
    final key = '$locationId|$varianceName';
