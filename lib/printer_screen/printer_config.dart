...
import 'package:yen_pos/printer_screen/screen/item_asign.dart';
...
class _PrinterSettingsScreenState extends State<PrinterSettingsScreen> {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: TextField(
            ...
            decoration: InputDecoration(hintText: '192.168.1.XXX'),
          ),
          actions: <Widget>[
            TextButton(
              child: Text('Cancel'),
            ...
            TextButton(
              ...
              onPressed: () {
                if (_ipTextController.text.isNotEmpty) {
                  setState(() {
                    Provider.of<PrinterProviderpos>(
                      context,
                      listen: false,
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Text('ADD PRINTER', style: TextStyle(fontSize: 18)),
          content: Column(
  ...
  void _showRemovePrinterDialog(
    ...
    PrinterProviderpos printerProvider,
    int index,
  ) {
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
            'Are you sure you want to remove this printerIp?',
          ),
          actions: [
            ElevatedButton(
              onPressed: () {
...
class IpInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      // Prevent empty middle octets (..)
      if (part.isEmpty && i != parts.length - 1) {
        return oldValue;
      }

      // Max 3 digits per octet
      if (part.length > 3) {
        return oldValue;
      }
