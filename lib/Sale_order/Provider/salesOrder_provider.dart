...
class SaleOrderProvider with ChangeNotifier {
  ...
  SaleOrderProvider() {
    defaultColumns = columnVisibility.keys
        .where((key) => columnVisibility[key]!)
        .toList();
    fetchOrders();
  }
  ...
  void filterItemsByQuery(String query) {
    if (query.isEmpty) {
      ...
      _filteredItems = _allItems
          .where(
            (item) => (item['varianceName'] ?? '').toLowerCase().contains(
              query.toLowerCase(),
            ),
          )
          .toList();
                        ...
                        child: ConstrainedBox(
                          ...
                          child: DataTable(
                            columns: defaultColumns
                                ...
                                .toList(),
                            rows: List.generate(submittedOrders.length, (
                              index,
                            ) {
