...
import 'package:yenposapp/screens/more_page/model/shift_model.dart';
import 'package:yenposapp/screens/sales_order/globals.dart';
import 'package:yenposapp/screens/shift_managment_page/openshift/open_shift.dart';
...
class CashManagementProvider extends ChangeNotifier {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, String>> formattedShifts =
            (shifts as List<dynamic>).map<Map<String, String>>((shift) {
  ...
  Future<List<Map<String, String>>> fetchDayEndDetails() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data is List) {
        ...
        if (kDebugMode) {
          debugPrint('Unexpected response data format or status code: '
              '${response.statusCode}');
        }
        return [];
      }
  ...
  Future<void> printOpenDayEndShifts(
      Map<String, dynamic> dayEnd, List<Map<String, dynamic>> shifts) async {
    debugPrint(
    ...
    allBytes +=
        generator.text("Device Number: ${dayEnd['deviceNumber'] ?? 'N/A'}");
    ...
    allBytes += generator.hr();
    allBytes += generator
        .text("Other Sales: ${dayEnd['otherSales']?.toString() ?? 'N/A'}");
    allBytes += generator.hr();
    allBytes += generator
        .text("Sales Return: ${dayEnd['salesReturn']?.toString() ?? 'N/A'}");
    ...
    for (var shift in shifts) {
      ...
      // Overall Sales
      allBytes += generator.text("OverAll Sales",
  ...
  static Future<void> printShiftData(Map<String, dynamic> shift) async {
    ...
    bytes += generator.text("Petty Cash",
        styles: PosStyles(bold: true, align: PosAlign.center));
    bytes += generator.hr();
    ...
    bytes += generator.hr();
    bytes += generator.text("Cash Sales",
        styles: PosStyles(bold: true, align: PosAlign.center));
    bytes += generator.hr();
    bytes += generator.row([
      PosColumn(
    ...
    bytes += generator.hr();
    bytes += generator.text("Card Sales",
        styles: PosStyles(bold: true, align: PosAlign.center));
    bytes += generator.hr();
    ...
    bytes += generator.hr();
    bytes += generator.text("UPI Sales",
        styles: PosStyles(bold: true, align: PosAlign.center));
    bytes += generator.hr();
    ...
    bytes += generator.text("Other Sales",
        styles: PosStyles(bold: true, align: PosAlign.center));
    bytes += generator.hr();
    bytes += generator.row([
      PosColumn(
    ...
    bytes +=
        generator.text("Day End Status: ${shift['dayEndStatus'] ?? 'N/A'}");
  ...
  static Future<void> postDayEndData(
      Map<String, dynamic> dayEndPost, context) async {
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('DayEnded successfully!')),
        );
      }
  ...
  static Future<void> printStoreReceipt(String ip) async {
    ...
    final PosPrintResult connectionResult =
        await printer.connect(ip, port: 9100);
    if (connectionResult == PosPrintResult.success) {
      final generator = Generator(PaperSize.mm80, profile);
      final bytes = <int>[];
      bytes.addAll(generator.text('BestMummy',
          styles: PosStyles(align: PosAlign.center, bold: true)));
      bytes.addAll(generator.text('Sweets & Cakes',
          styles: PosStyles(align: PosAlign.center)));
  ...
  static String calculateDifference(
      String? systemClosing, String? manualClosing) {
