...
class UnprintedOrdersProvider extends ChangeNotifier {
  ...
  Future<void> fetchUnprintedOrders() async {
    ...
    final orders = rawList.map<Map<String, dynamic>>((raw) {
      ...
      return rawMap.map<String, dynamic>((k, v) => MapEntry(k.toString(), v));
    }).toList();
  ...
  Future<void> retryPrint(
    ...
    try {
      ...
      List<Map<String, dynamic>> seatOrders = _parseSeatOrders(
        order["seatOrders"],
      );
  ...
  List<Map<String, dynamic>> _parseSeatOrders(dynamic rawSeatOrders) {
    if (rawSeatOrders is String) {
      try {
        return (jsonDecode(rawSeatOrders) as List)
      ...
      return rawSeatOrders
          .map((item) => Map<String, dynamic>.from(item as Map))
          .toList();
    } else {
      throw Exception(
        "Unexpected seatOrders type: ${rawSeatOrders.runtimeType}",
      );
    }
  }
