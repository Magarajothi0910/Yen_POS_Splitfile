...
import 'package:yen_pos/Sale_order/Print_Receipt/currentOrderPrint.dart';
...
import 'package:yen_pos/Sale_order/Screens/editcustomerdetails.dart';
import 'package:yen_pos/Sale_order/Widgets/add_advance_dialogue.dart';

import 'package:yen_pos/Sale_order/Widgets/cancel_order_dialogue.dart';
import 'package:yen_pos/Sale_order/Widgets/date_button.dart';
...
class _AllOrdersPageState extends State<AllOrdersPage> {
                          ...
                          CustomButton(
                            ...
                            onPressed: () async {
                              if (salesOrder.status == "received") {
                                ...
                                var branchwiseItems =
                                    lazyBox.get(
                                          'branchwiseItems_${globals.locationId}',
                                        )
                                        as Map?;
  ...
  void _showAddItemDialog(
    BuildContext context,
    EditCustomerScreenProvider customerScreenProvider,
    SaleOrderRegularModeProvider regularModeProvider,
  ) {
      ...
      searchResults = regularModeProvider.originalItems
          ...
          .where((variance) {
            final name = (variance['varianceName']?.toString() ?? '')
                .toLowerCase();
            final code = (variance['itemCode']?.toString() ?? '');
                                  ...
                                  child: ListTile(
                                    ...
                                    subtitle: Column(
                                      ...
                                      children: [
                                        ...
                                        const SizedBox(height: 4),
                                        Text(
                                          '${item['varianceUom'] ?? ''} • ₹${item['variancePrice']?.toStringAsFixed(2) ?? '0.00'} • Code: ${item['itemCode'] ?? ''}',
                                          style: TextStyle(
                                            color: Colors.grey.shade600,
                                            fontSize: 13,
                                          ),
