...
Future<void> handleNewClientConnected(Map<String, dynamic> data, WebSocketChannel channel) async {
  try {
  ...
  for (var data in _receivedData) {
    try {
      if (data['action'] == 'seat_transfer' ||
          (data.containsKey('date') && data['date'] == currentDate && (data['status'] == "active" || data['status'] == "confirm" || data['status'] == "invoiced" || data['status'] == "cancelled"))) {
...
Future<void> sendDataToClientsKOT(Map<String, dynamic> data) async {
  try {
    ...
    print("📤 Data sent to ${clients.length} clients in ${after.difference(before).inMilliseconds} ms");
  } catch (e, st) {
...
Future<void> handleRemovePrinter(Map<String, dynamic> data) async {
  try {
    ...
    _receivedData.removeWhere((entry) => entry['action'] == 'printerDetails' && entry['name'] == printerName);
    var printerBox = Hive.box('printerData');
    await printerBox.delete(printerName);

    print("🗑️ Removed printer: $printerName");

    await sendDataToClientsKOT({'action': 'removePrinter', 'name': printerName});

    final updatedPrinters = printerBox.values.toList();
    await sendDataToClientsKOT({'action': 'allPrinterDetails', 'printers': updatedPrinters});
...
Future<void> sendAllDataToClient(WebSocketChannel channel, {required bool isUpiEnabled}) async {
  try {
    ...
    final filteredInvoices = invoices.where((invoice) {
      try {
        final invoiceDate = DateFormat('dd-MM-yyyy').parse(invoice['invoiceDate']);
