...
class _PrinterSettingsScreenState extends State<PrinterSettingsScreen> {
  ...
  Future<void> _initializeProviders() async {
    try {
      ...
      print('✅ Printer provider initialized: ${printerProvider.printers.length} printers');
      setState(() {
        _isInitialized = true;
      });
    } catch (e) {
  ...
  Future<void> _addPrinter(PrinterProviderDine printerProvider, BuildContext context) async {
    try {
      ...
      // Verify the printer was added
      print("🔍 Current printers after adding: ${printerProvider.printers.length}");
            ...
            builder: (context, setDialogState) {
              return AlertDialog(
                ...
                actions: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      await _updatePrinter(printerProvider, webSocketService, index, context);
                    },
                    style: ElevatedButton.styleFrom(
                      elevation: 2,
                      shape: RoundedRectangleBorder(
  ...
  Future<void> _updatePrinter(PrinterProviderDine printerProvider, WebSocketService webSocketService, int index, BuildContext context) async {
    try {
      final name = _nameController.text.trim();
      final ip = _ipController.text.trim();
      ...
      final existingPrinterIndex = printerProvider.printers.indexWhere((p) => p.name == name && p.name != originalPrinter.name);
                                      ...
                                      Padding(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                              ...
                                              ElevatedButton.icon(
                                                onPressed: () {
                                                  ...
                                                  try {
                                                    ...
                                                    print('❌ Failed to navigate to ItemAssignmentScreen: $e');
                                                    showCustomFlushbar(
                                                      context,
                                                      'Failed to open item assignment',
                                                      type: FlushbarType.error,
                                            ...
                                            IconButton(
                                              icon: const Icon(Icons.delete, color: Colors.red),
                                              onPressed: () => _showRemovePrinterDialog(context, index),
                                              tooltip: 'Delete Printer',
                                            ),
                                          ],
                                        ),
