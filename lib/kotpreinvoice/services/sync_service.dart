...
class SyncServiceKot {
  // final String apiUrl = 'https://yenerp.com/masteradminapi/orders/';
  ...
  final String kotTableStatusUrl = 'https://yenerp.com/masteradminapi/kottablesstatus';
  ...
  Future<void> patchEditedOrders() async {
    try {
      print("Starting patchEditedOrders()");

      final ordersBox = Hive.isBoxOpen('ordersBox')
          ? Hive.box('ordersBox')
          : await Hive.openBox('ordersBox');

      if (ordersBox.isEmpty) return;

      // IMPORTANT: iterate backwards (safe delete)
      for (int i = ordersBox.length - 1; i >= 0; i--) {
        try {
          dynamic orderData = ordersBox.getAt(i);
          ...
          // Only synced & edited orders
          if (orderData['sync'] != 'Yes' || orderData['edit'] != 'Yes')
            continue;

          // Skip if already in progress
          if (orderData['patchInProgress'] == true) continue;

          orderData['patchInProgress'] = true;
          await ordersBox.putAt(i, orderData);

          final hiveOrderId = orderData['hiveOrderId']?.toString();
          final seathiveOrderId = orderData['seathiveOrderId']?.toString();

          /* ---------------- FIELD PATCH ---------------- */
          if (orderData['fieldsEdited'] == 'true' && hiveOrderId != null) {
            final ok = await patchFieldsByHiveOrderId(hiveOrderId, orderData);
            ...
            if (ok) {
              ...
              _queueFieldRetry(hiveOrderId);
            }
          }

          /* ---------------- STATUS PATCH ---------------- */
          if (orderData['statusEdited'] == 'true' && seathiveOrderId != null) {
            final status =
                orderData['status']?.toString().toLowerCase() ?? 'unknown';

            final ok = await patchOrderStatusWithRetry(seathiveOrderId, status);

            if (ok) {
              if (status == 'invoiced') {
                await ordersBox.deleteAt(i);
              } else {
                orderData['statusEdited'] = 'false';
                orderData['edit'] = 'No';
                orderData['patchInProgress'] = false;
              ...
              _queueStatusRetry(seathiveOrderId, status);
            }
          }
        } catch (e) {
      ...
      print("Completed patchEditedOrders()");
    } catch (e, st) {
      print("Fatal error in patchEditedOrders(): $e");
      print(st);
    }
  }

  void _queueFieldRetry(String hiveOrderId) {
    queueSync(() async {
      final box = Hive.box('ordersBox');

      final index = box.values.toList().indexWhere(
        (e) => e is Map && e['hiveOrderId']?.toString() == hiveOrderId,
      );

      if (index == -1) return;

      final latest = Map<String, dynamic>.from(box.getAt(index));
      if (latest['fieldsEdited'] != 'true') return;

      latest['patchInProgress'] = true;
      await box.putAt(index, latest);

      final ok = await patchFieldsByHiveOrderId(hiveOrderId, latest);

      if (ok) {
        latest['fieldsEdited'] = 'false';
        latest['edit'] = 'No';
      }

      latest['patchInProgress'] = false;
      await box.putAt(index, latest);
    });
  }

  void _queueStatusRetry(String seathiveOrderId, String status) {
    queueSync(() async {
      final box = Hive.box('ordersBox');

      final index = box.values.toList().indexWhere(
        (e) => e is Map && e['seathiveOrderId']?.toString() == seathiveOrderId,
      );
      ...
      final ok = await patchOrderStatusBySeathiveOrderId(
        seathiveOrderId,
        status,
      );
