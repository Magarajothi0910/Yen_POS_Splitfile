...
//   //       await startUdpResponder(savedIp, udpPort);
//   //       startServer(clients, onDataReceived);
//   //       print("Restarted server after app reopen.");
//   //     } else {
//   //       // Otherwise, it’s running as a client
//   //       appType = 'client';
...
//   //       serverFound = true;
//   //     });
//   //     print("Using previously stored server IP: $serverip:$serverPort");
//   //   } else {
//   //     setState(() {
//   //       serverFound = false;
//   //     });
...
//               // // 🔍 print right after setting it
//               // print(
//               //     '🖨️ [per‐variance] $itemKey → $varKey → $alias.localStock = '
//               //     '${entry['localStock']}');
//             }
...
//         // 1) Open the Hive box and deserialize
//         final box = await Hive.openBox('branchwise_items');
//         final raw = Map<String, dynamic>.from(box.get('data') as Map);
//         final items = Map<String, dynamic>.from(raw['data'] as Map);
...
//           final code = u['varianceitemCode']?.toString() ?? '';
//           final qty = (u['quantity'] as num?)?.toDouble() ?? 0.0;

//           // Find the matching variance in our nested map
//           items.forEach((itemKey, itemValue) {
//             final item = Map<String, dynamic>.from(itemValue as Map);
//             final variances =
//                 Map<String, dynamic>.from(item['variance'] as Map);

//             variances.forEach((varKey, varValue) async {
//               final vmap = Map<String, dynamic>.from(varValue as Map);
//               if (vmap['varianceitemCode']?.toString() == code) {
//                 // Extract branchwise entry
//                 final branchwise =
//                     Map<String, dynamic>.from(vmap['branchwise'] as Map);
//                 if (branchwise.containsKey(alias)) {
//                   final entry =
//                       Map<String, dynamic>.from(branchwise[alias] as Map);

//                   // Compute new stock
//                   final oldStock =
//                       (entry['localStock'] as num?)?.toDouble() ?? 100.0;
//                   final newStock = max(0.0, oldStock - qty);

//                   // Persist into Hive
//                   await _setLocalStockInHive(
//                     varianceCode: code,
//                     newLocalStock: newStock,
//                   );

//                   // Update in-memory structure for this session
//                   entry['localStock'] = newStock;
//                   branchwise[alias] = entry;
//                   vmap['branchwise'] = branchwise;
//                   variances[varKey] = vmap;

//                   // Log it
//                   print('🖨️ variance:$code  branch:$alias  '
//                       'localStock:${entry['localStock']}  newStock:${newStock.toInt()}');
//                 }
//               }
//             });

//             // Write back the modified variances into the item
//             item['variance'] = variances;
//             items[itemKey] = item;
//           });
//         }

//         // 4) Persist the entire updated map back to Hive
//         raw['data'] = items;
//         await box.put('data', raw);

//         print('✅ updateLocalStock applied: $updates');
//       },
