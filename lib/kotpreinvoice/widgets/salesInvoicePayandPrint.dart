...
class _SalesInvoicePayAndPrintState extends State<SalesInvoicePayAndPrint> {
  ...
  void initState() {
    ...
    if (widget.customerName.isNotEmpty) {
      _customerNumberController.text = widget.customerName;
    }
    ...
    // debugPrint("_generateCashOptions is $cashOptions");
  }
  ...
  double getRemainingForMethod(String method) {
    ...
    if (method != 'Cash') paidByOthers += state.cashAmount;
    if (method != 'UPI') paidByOthers += state.upiAmount;
    if (method != 'Card') paidByOthers += state.cardAmount;

    return (total - paidByOthers).clamp(0.0, double.infinity);
  }
  ...
  //     // }
  //     if (method == "Cash") {
  //       double digitalPaid = currentUpi + currentCard;
  //       double maxAllowedCash = widget.totalAmount - digitalPaid;

  //       if (digitalPaid >= widget.totalAmount - 0.01) {
  //         // Digital already covers full → force cash = 0
  //         enteredAmount = 0.0;
  //         _showCustomSnackBar(
  //           "Full amount already paid via UPI/Card. Cash not needed.",
  //           isSuccess: false,
  //         );
  //       } else if (enteredAmount > maxAllowedCash + 0.01) {
  //         enteredAmount = maxAllowedCash;
  //         _showCustomSnackBar(
  //           "Cash limited to remaining ₹${maxAllowedCash.toStringAsFixed(0)}",
  //           isSuccess: false,
  //         );
  //       }
  //     }
  ...
  // }
  void _selectPaymentOption(String method, String amountStr) {
    double entered = double.tryParse(amountStr) ?? 0.0;
    ...
    if (method == "Cash") {
      // ────────────────────────────────────────────────
      // Rule: Cash can be > total ONLY if UPI + Card == 0
      // ────────────────────────────────────────────────
      double digitalPaid = currentUpi + currentCard;

      if (digitalPaid >= 0.01) {
        // Some digital payment already exists → strict limit
        double maxAllowed = widget.totalAmount - digitalPaid;
        if (entered > maxAllowed + 0.01) {
          entered = maxAllowed.clamp(0.0, double.infinity);
          _showCustomSnackBar(
            "Cash limited to remaining ₹${maxAllowed.toStringAsFixed(0)}",
            isSuccess: false,
          );
        }
      }
      // else → both UPI & Card are ~0 → allow any amount (even > total)

      currentCash = entered;
      _customCashController.text = entered.toStringAsFixed(0);
    } else if (method == "UPI") {
      double maxAllowed = getRemainingForMethod("UPI");
      if (entered > maxAllowed + 0.01) {
        entered = maxAllowed;
        _showCustomSnackBar(
          "UPI cannot exceed remaining ₹${maxAllowed.toStringAsFixed(0)}",
          isSuccess: false,
        );
      }
      currentUpi = entered;
      _customUpiController.text = entered.toStringAsFixed(0);
    } else if (method == "Card") {
      double maxAllowed = getRemainingForMethod("Card");
      if (entered > maxAllowed + 0.01) {
        entered = maxAllowed;
        _showCustomSnackBar(
          "Card cannot exceed remaining ₹${maxAllowed.toStringAsFixed(0)}",
          isSuccess: false,
        );
      }
      currentCard = entered;
      _customCardController.text = entered.toStringAsFixed(0);
    }
    ...
    // Refresh cash quick options
    cashOptions.clear();
    cashOptions.addAll(_generateCashOptions(state.balanceAmount));
  }
  ...
  //   return options;
  // }
  // 1. Update _generateCashOptions – return empty list (or only ['0']) when digital exists
  List<String> _generateCashOptions(double remaining) {
    final digitalPaid = state.upiAmount + state.cardAmount;

    if (digitalPaid > 0.01) {
      // When UPI or Card has value → no fancy cash buttons
      // You can still allow manual entry in the text field if you want
      return ['0']; // or return [] if you want ZERO buttons
    }

    // No digital payment → normal rounding + higher options
    if (remaining <= 0) return ['0'];

    List<String> options = [];
    int exact = remaining.ceil();
    options.add(exact.toString());

    final denominations = [10, 20, 50, 100, 200, 500, 1000, 2000];

    for (int d in denominations) {
      int rounded = ((exact + d - 1) ~/ d) * d;
      if (rounded > exact && rounded <= 5000) {
        options.add(rounded.toString());
      }
    }
  ...
  Future<void> _createOrderAndPay() async {
    ...
    try {
      ...
      final response = await _dio.post(
        "/masteradminapi/razorPay/create_order/?price=$amount",
      );
  ...
  //       "https://yenerp.com/masteradminapi/razorPay/verify_payment",
  //       data: verifyData,
  //     );
  ...
  Widget _buildPaymentSection(String method) {
    ...
    // For Cash: we generate options differently now
    List<String> cashQuickOptions = [];
    bool showQuickCashButtons = false;

    if (method == 'Cash') {
      cashQuickOptions = _generateCashOptions(remaining);
      // Show buttons ONLY if no digital payment is used
      showQuickCashButtons =
          (state.upiAmount + state.cardAmount <= 0.01) &&
          cashQuickOptions.length > 1; // more than just '0'
    } else {
      // UPI & Card always show exact + custom
      cashQuickOptions = [exactStr];
    }
      ...
      children: [
        Row(
          ...
          children: [
            ...
            // Always show the exact remaining button (for consistency)
            _buildPaymentOption(exactStr, method),
          ],
        ),

        // ────────────────────────────────────────────────
        // Only show horizontal quick buttons for Cash when allowed
        // ────────────────────────────────────────────────
        if (method == 'Cash' && showQuickCashButtons)
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(
              children: [
                for (var option in cashQuickOptions)
                  if (option != exactStr) // avoid duplicating the "exact" one
                    _buildPaymentOption(option, method),
              ],
            ),
          ),

        // // Optional: small hint when cash is restricted
        // if (method == 'Cash' && state.upiAmount + state.cardAmount > 0.01)
        //   Padding(
        //     padding: const EdgeInsets.only(left: 8, top: 4, bottom: 8),
        //     child: Text(
        //       "Cash buttons hidden – use custom field if needed",
        //       style: TextStyle(
        //         fontSize: 12,
    ...
    return MultiProvider(
      ...
      child: Scaffold(
        ...
        body: Consumer<SalesInvoicePayAndPrintState>(
          builder: (context, state, _) {
            bool cashIsDisabled =
                (state.upiAmount + state.cardAmount >=
                widget.totalAmount - 0.01);

            bool upiIsDisabled =
                (state._cashAmount + state.cardAmount >=
                widget.totalAmount - 0.01);

            bool cardIsDisabled =
                (state.upiAmount + state._cashAmount >=
                widget.totalAmount - 0.01);
                            ...
                            children: [
                              Padding(
                                ...
                                child: Material(
                                  ...
                                  borderRadius: BorderRadius.circular(8),
                                  child: Opacity(
                                    opacity: cashIsDisabled ? 0.55 : 1.0,
                                    child: IgnorePointer(
                                      ignoring: cashIsDisabled,
                                      child: Padding(
                                        padding: const EdgeInsets.only(
                                          left: 10,
                                        ),
                              ...
                              Padding(
                                ...
                                child: Material(
                                  ...
                                  borderRadius: BorderRadius.circular(8),
                                  child: Opacity(
                                    opacity: upiIsDisabled ? 0.55 : 1.0,
                                    child: IgnorePointer(
                                      ignoring: upiIsDisabled,
                                      child: Padding(
                                        padding: const EdgeInsets.only(
                                          left: 10,
                                        ),
                              ...
                              Padding(
                                ...
                                child: Material(
                                  ...
                                  borderRadius: BorderRadius.circular(8),
                                  child: Opacity(
                                    opacity: cardIsDisabled ? 0.55 : 1.0,
                                    child: IgnorePointer(
                                      ignoring: cardIsDisabled,
                                      child: Padding(
                                        padding: const EdgeInsets.only(
                                          left: 10,
                                        ),
                              ...
                              return Consumer<SalesInvoicePayAndPrintState>(
                                builder: (context, state, _) {
                                  final String customerText = customerValue.text
                                      .trim();
                                  final bool isCustomerFilled =
                                      customerText.length >= 10; // or your rule
                                  final bool isFullyPaid =
                                      state.balanceAmount <= 0;
                                  final bool hasSalesPerson =
                                      _employeeNumberController.text
                                          .trim()
                                          .isNotEmpty;

                                  // ────────────────────────────────────────────────
                                  // NEW: Check if any payment method has amount > 0
                                  // ────────────────────────────────────────────────
                                  final bool hasAnyPayment =
                                      state.cashAmount > 0 ||
                                      state.upiAmount > 0 ||
                                      state.cardAmount > 0;
                                  ...
                                  return ElevatedButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (state.balanceAmount > 0) {
                                        CustomSnackBar.show(
                                          context,
                                          'Balance amount is still remaining',
                                          type: SnackType.error,
                                        );
                                        return;
                                      }
  ...
  void _sendInvoiceDataToServer(double balanceAmount) {
    ...
    for (var item in widget.items) {
      ...
      for (int i = 0; i < (item['varianceName']?.length ?? 0); i++) {
        ...
        double weight = (item['weight']?[i] ?? 0.0).toDouble();
        String uom = item['uom']?[i] ?? "kgs";

        // 🔑 KEY RULE
        // kgs → always new line
        // pcs → merge by variance
        String key = uom == 'pcs'
            ? "$variance-$weight"
            : "$variance-$weight${DateTime.now().microsecondsSinceEpoch}";
        ...
        // ✅ Qty increases ONLY for PCS
        if (uom == 'pcs') {
          groupedItems[key]!['qty'] += item['qty']?[i] ?? 0.0;
          groupedItems[key]!['amount'] += item['amount']?[i] ?? 0.0;
        } else {
          // kgs → single entry per line
          groupedItems[key]!['qty'] = item['qty']?[i] ?? 0.0;
          groupedItems[key]!['amount'] = item['amount']?[i] ?? 0.0;
        }
    ...
    Map<String, dynamic> invoiceData = {
      ...
      "varianceitemCode": groupedItems.values
          ...
          .toList(),
      "varianceName": groupedItems.keys
          .map((key) => key.toString().split('-').first.trim())
          .toList(),
      ...
      "totalAmount": widget.totalAmount,
      "sellingPrice": groupedItems.values.map((item) => item['price']).toList(),
    ...
    sendataToServer(invoiceData);
  }

  void _processInvoiceAndPrint(double balanceAmount) async {
    try {
      _sendInvoiceDataToServer(balanceAmount);
