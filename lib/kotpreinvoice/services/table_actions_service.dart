...
import 'package:yenpos/kotpreinvoice/services/websocketService.dart';
...
class TableActionsService {
  static Future<void> sendSeatActionToServer({
    ...
    try {
      ...
      debugPrint("🟢 Preparing to send seat action: Table=$tableNumber, Seat=$seat, Area=$areaName");

      List<Map<String, dynamic>> ordersForSeat = [];
      try {
        ordersForSeat = orderProvider
            .getActiveOrdersForSeat(tableNumber, seat)
            .where((order) => order['table'] == tableNumber && order['seat'] == seat)
            .toList();
      ...
      try {
        ...
        if (holdOrder != null) {
          ...
          debugPrint("ℹ️ No hold order found for table $tableNumber seat $seat");
        }
      } catch (e, stack) {
        ...
        Future.delayed(const Duration(milliseconds: 100), () {
          try {
            ...
            if (navigationAreaName.isEmpty) {
              debugPrint("❌ No area found for table $tableNumber, cannot navigate");
              if (context.mounted) {
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  showCustomFlushbar(
                    context,
            ...
            debugPrint("➡️ Showing ProductCardScreen as overlay for Table=$tableNumber, Seat=$seat, Area=$navigationAreaName");
          } catch (e, stack) {
  ...
  static String _getAreaNameForTable(BuildContext context, String tableNumber) {
    try {
      ...
      debugPrint('❌ Error in _getAreaNameForTable for $tableNumber: $e\n$stack');
      return '';
    }
  }
}