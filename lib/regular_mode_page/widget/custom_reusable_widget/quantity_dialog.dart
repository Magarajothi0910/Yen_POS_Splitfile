...
import 'package:yen_pos/regular_mode_page/widget/custom_reusable_widget/scafflodMesseger.dart';
                    ...
                    Material(
                      ...
                      child: SizedBox(
                        ...
                        child: TextField(
                          ...
                          onChanged: (value) {
                            debugPrint(
                              "📝 Quantity TextField changed to: '$value'",
                            );
                            if (value.isEmpty) {
                              return;
                            }
            ...
            builder: (ctx, provider, _) {
              return TextButton(
                onPressed: () async {
                  ...
                  debugPrint(
                    "   systemStockOverride provided: $systemStockOverride",
                  );
                  ...
                  if (systemStock <= 0) {
                    ...
                    try {
                      ...
                      debugPrint(
                        "   Hive key used: branchwiseItems_$locationId",
                      );
                      ...
                      debugPrint(
                        "   branchwiseItems['data'] keys: ${branchwiseItems?.keys.join(', ')}",
                      );

                      final itemData = branchwiseItems?[itemName];
                      debugPrint(
                        "   Lookup itemName: '$itemName' → Found: ${itemData != null}",
                      );

                      if (itemData != null) {
                        final varianceMap = itemData['variance'] as Map?;
                        debugPrint(
                          "   Variance map keys: ${varianceMap?.keys.join(', ')}",
                        );

                        final varianceData = varianceMap?[varianceName];
                        debugPrint(
                          "   Lookup varianceName: '$varianceName' → Found: ${varianceData != null}",
                        );
                        ...
                        if (varianceData != null) {
                          ...
                          systemStock =
                              (branchStock as num?)?.toDouble() ?? 0.0;
                          debugPrint("   → Fetched systemStock: $systemStock");
                        } else {
                          debugPrint(
                            "   → varianceData not found for '$varianceName'",
                          );
                        }
                      } else {
                  ...
                  debugPrint(
                    "   Final systemStock used for validation: $systemStock",
                  );
                  debugPrint(
                    "   Comparing: selectedQty ($selectedQty) > systemStock ($systemStock)? ${selectedQty > systemStock}",
                  );
                  ...
                  debugPrint(
                    "   → Closing dialog and calling onAddToCart($selectedQty)",
                  );
