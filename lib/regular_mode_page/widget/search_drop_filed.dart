...
import 'package:yen_pos/regular_mode_page/widget/custom_reusable_widget/quantity_dialog.dart';
...
class _SearchDropdownState extends State<SearchDropdown> {
  ...
  Widget _buildDropdownContent() {
    return Consumer<RegularModeProvider>(
              ...
              ? ListView.builder(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    return ListTile(
                      ...
                      onTap: () async {
                        final selectedItem = provider.getVarianceDetails(
                          varianceName,
                        );
  ...
  Future<String?> _getItemNameForVariance(
    String varianceName,
    String locationId,
  ) async {
    // Open the Hive box
    ...
    final branchwiseData =
        (await lazyBox.get('branchwiseItems_$locationId'))?['data']
    ...
    for (var entry in branchwiseData.entries) {
      ...
      if (itemData != null && itemData['variance'] != null) {
        ...
        if (varianceMap.values.any(
          (v) =>
              (v as Map<dynamic, dynamic>)['varianceName']?.toString() ==
              varianceName,
        )) {
          return itemName;
        }
    ...
    print(
      'DEBUG: _getItemNameForVariance - No item found for variance: $varianceName',
    );
    return null;
  }
  ...
  Future<void> _handleItemSelection(
    ...
    // --- Fetch from Hive ---
    final lazyBox = await Hive.openBox('items');
    final branchwiseItemsData = await lazyBox.get(
    ...
    if (branchwiseItems != null && branchwiseItems.containsKey(itemName)) {
      final rawItemData =
          branchwiseItems[itemName]?['item'] as Map<dynamic, dynamic>?;
      if (rawItemData != null) {
        itemData = rawItemData.map(
          (key, value) => MapEntry(key.toString(), value),
        );
        tax = (rawItemData['tax'] as num?)?.toDouble() ?? 0.0;
      }
      systemStock =
          (varianceData['branchwise']?['${locationId}']?['systemStock'] as num?)
              ?.toDouble() ??
          0.0;
    }
    ...
    Map<String, dynamic> itemToAdd = {
      'itemData': {
        ...
        'category':
            ...
            varianceData['category']?.toString() ??
            'Unknown',
      },
      'varianceData': {
    ...
    void _addDirectlyToCart() {
      if (1 > systemStock) {
        _showErrorSnackbar(
          context,
          "Insufficient stock for $varianceName. Available: $systemStock.",
        );
        return;
      }
          ...
          actions: <Widget>[
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                print(
                  'DEBUG: _showBluetoothWeightDialog - Adding weight item: $itemToAdd',
                );
                saleProvider.addItemToCart(itemToAdd);

                _showSuccessSnackbar(
                  dialogContext,
                  "$varianceName (${weight.toStringAsFixed(3)} Kg)",
                );
                Navigator.of(dialogContext).pop();
              },
              style: ButtonStyle(
      ...
      builder: (dialogContext) {
        return NumericCalculator(
          ...
          onValueSelected: (weight) {
            ...
            itemToAdd['totalPrice'] = weight * price;
            print(
              'DEBUG: _showNumericCalculatorDialog - Adding weight item: $itemToAdd',
            );
            saleProvider.addItemToCart(itemToAdd);
            _showSuccessSnackbar(
              dialogContext,
              "$varianceName (${weight.toStringAsFixed(2)} Kg)",
            );
          },
        );
      },
    ...
    try {
      ...
      if (scannedData.containsKey('ItemCode')) {
        ...
        if (result.isNotEmpty) {
          ...
          final varianceName =
              itemData['varianceData']['varianceName']?.toString() ?? '';
          final selectedItem = provider.getVarianceDetails(varianceName);
          if (selectedItem != null) {
            final varianceData = provider.getVarianceDetails(varianceName);
            final itemName =
                await _getItemNameForVariance(varianceName, locationId) ??
                'Unknown Item';
            final varianceUOM =
                provider.getUOMForVariance(varianceName)?.toString() ??
                'Unknown UOM';
            ...
            if (quantity > systemStock) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    "Scanned quantity (${quantity.toInt()}) exceeds available stock (${systemStock.toInt()}).",
                  ),
                  backgroundColor: Colors.red,
                  duration: const Duration(seconds: 2),
                ),
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text("Item details not found."),
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        suffixIcon: IconButton(
                          onPressed: () {
                            _controller.clear();
                            // _updateOverlay(); // hide/update overlay if used
